...
def eval_confusion(model, loader, device, threshold = 0.5):
    ...
    print(f"\m--Final Evaluation (threshold={threshold}) --")
...
class CNNTransformerHybridPatient(nn.Module):
    def __init__(self, d_model=512, nhead=8, num_layers=2, dim_feedforward=1024, dropout=0.3, load_pretrained=True, freeze_backbone=True, unfreeze_layer4=True, resnet_weight_path=LOCAL_RESNET_WEIGHTS):
        super().__init__()
...
#8 공통 학습 함수
def fit_model(model, model_name, fold_idx, train_loader, val_loader, device, num_epochs=30, lr = 1e-4, weight_decay=1e-4, pos_weight=None, patience = 7, min_delta=0.001):
